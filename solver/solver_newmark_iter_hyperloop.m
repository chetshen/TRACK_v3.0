...
function [acc2,vel2,dis2,F,Z,mc_ws2, I_2]=solver_newmark_iter_hyperloop(mat_trk,inp,shape, Z, wh_ld, acc1, vel1, dis1,mc_ws1,X_w_t, geo,Z_global,contactID,Fex, mat_ws,I_1,mat_vhcl, Y)
%initial condition
...
delta_ss = 0.015;
C = 0.05;
I_ss = sqrt(m_w*9.8*delta_ss.^2/C);
R = 0.6;
kp = 20e3;
kd = 40e3;
...
switch contactID
    ...
    case 111
        ...
        F = 0;
    case 1000 %hyperloop
penetration = Z.w-Z.r-Z.irr + delta_ss;
% F_tr = 0;
I_tr_0 = I_1 - I_ss;
F_tr = 2.*C.*(I_ss.^2)./(delta_ss.^3).*(delta_ss./I_ss.*I_tr_0 + penetration);
F = F_tr + m_w*9.8;
...
%             % !!      initial position of the wheel X_w0
        ...
        case 111
            ...
            [F,~,~]=Fz_kik(Z,Y);
        case 1000 %hyperloop
penetration = Z.w-Z.r-Z.irr+delta_ss;%u_tr-w0_tr
...
%     dIdt = delta_ss/(2*C) * (-R*I + kp * (-1*penetration) + (kd + 2*C*I_ss/(delta_ss.^2)*((shape*vel2.r')'-vel2.w))); 
%     end
