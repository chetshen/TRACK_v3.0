%%
%Parameter to match "Distributed support modelling for vertical track dynamic analysis, Blanco, 2018"
% "Implementation of Timoshenko element local deflection for vertical track modelling, Blanco et al,2018"
%%
function [in_data,NNslpf] = get_input_Blanco_2018(in_data)
%%
%Geometry input
...
in_data.ext_force.wh_ld = 100e3 ; % 8000*9.8 ;% 12742*9.8;   %[N]
...
in_data.mater(1).Data=[210e9;    %E_R[N/m^2]
                    0.3055e-4;    %I_R[m^4] EI = 6.4e6 Nm^2
...
in_data.mater(3).Data=[350e6;%70e6; %K_Spring_RS [N/m]
                      4.8e4];%1.2e4]; %C_Damper_RS[N.s/m]
...
in_data.mater(4).Data = [70e6/NNslph;  %3e7 K_Spring_SB[N/m]
                         47e3/NNslph];%3.1e4];  %C_Damper_SB[N.s/m]
in_data.mater(4).Note='ballast';


%CONTACT
% in_data.mater(5).ElemType=5;   %5 for contact
in_data.mater(5).Data = 10e10;%hertz_stiff(0.45,Inf,0.3,210e9,0.3,210e9,0.3);%8.4e10;% 8.7e10; %C_Hertz %2/3*in_data.mater.E_R/(1-0.27^2)*sqrt(in_data.geo.Rw) ;% 8.7e10; %[N/m^(3/2)] from (Steffens, 2005)
in_data.mater(5).Note='contact';
%VEHICLE
% in_data.mater(6).ElemType=6;   %6 for vehicle
in_data.mater(6).Data =  [0;   %M_sprg%4653.5[kg]
                      550;%883.6; %M_unsprg[kg]
                     1.22e6]; %K_PS[N/m]
